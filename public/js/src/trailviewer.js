...
export const DefaultTrailViewerOptions = {
    useURLHashing: false,
...
export class TrailViewer {
    ...
    _navArrowMinAngle;
    _navArrowMaxAngle;
    _prevNavClickedYaw = 0;
    _initLat;
    _initLng;
    _imageFetchType;
    optimalDist = 4;
    neighborDistCutoff = 10;
    pruneAngle = 25;
    _firstScene = null;
    constructor(options, initImageId, data = null, lat = null, lng = null) {
        if (options === null) {
        ...
        if (data !== null) {
            ...
            this._fetchData().then((dataArr) => {
                ...
                if (this._panViewer !== null) {
                    ...
                    this.goToImageID(this.getCurrentImageID(), true);
                }
            });
        }
     ...
     * */
    async _addNeighborsToViewer(neighbors, flipped = false) {
        let instance = this;
        for (let i = 0; i < neighbors.length; i++) {
            ...
            let pitch = -(max - min - (neighbors[i]['distance'] * (max - min)) / 9.0) +
                max;
            ...
            if (distance <= this.neighborDistCutoff) {
                ...
                for (let n = 0; n < neighbors.length; n++) {
                    ...
                    if (Math.abs(diff) < this.pruneAngle) {
                        if (Math.abs(this.optimalDist - distance) <
    ...
    _getNearestHotspot(yaw) {
        ...
        for (let i = 0; i < hotspots.length; i++) {
            ...
            nearestDiff = Math.abs(this._customMod(angle180to360(nearest.yaw) - yaw + 180, 360) -
                180);
            if (diff < nearestDiff) {
                nearest = hotspots[i];
                nearestDiff = diff;
    ...
    _onSceneChange(img) {
        ...
        let newYaw = (((this._prevNorthOffset - this._panViewer.getNorthOffset()) %
            360) +
            this._prevYaw) %
            360;
    ...
    getBearing() {
        return ((this._panViewer.getNorthOffset() +
    ...
    async goToImageID(imageID, reset = false) {
